...
```{r}
quest_res_df = gather(quest_res_df, "reaction_one", "reaction_two", "reaction_three", "reaction_four", key = "trial", value = "reaction")
View(quest_res_df)
```

```{r}
p1 <- ggplot(quest_res_df) + 
  geom_point(aes(x = time_stamp, y = reaaction)) +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Reaction times for different trials", x = "Trial 1", y = "Reaction times") +
  theme_economist() +
  transition_states(
    states = trial, # ? or reaction_one and so on ? 
    transition_length = 3,
    state_length = 2) + 
  enter_fade() +
  exit_shrink() +
