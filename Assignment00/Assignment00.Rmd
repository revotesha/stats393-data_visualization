...
Actaully these two still do not show both info. The last one though shows both 
deaths per scale and mean of deaths for each scale - that is, the expecation of 
number of deaths if an EF 5 hits is about 20 (which is also what the previous 
one means). 
...
tornadoes_left = ggplot(data = tornadoes) + 
  ...
  labs(title = "Tornado Count by Scale", y = "Tornado Count", x = NULL) + 
  theme(plot.title = element_text(hjust = 0.5),
...
tornadoes_right <- tornadoes %>%
  ...
  labs(x = NULL, y = "Deaths per EF Scale", title = "Average Deaths by Scale") +
  theme(plot.title = element_text(hjust = 0.5),
        axis.ticks.y = element_blank()) +
  scale_x_continuous(breaks = c(0,1,2,3,4,5),
                     labels = c("EF0   ", "EF1   ", "EF2   ", "EF3   ", "EF4   ", "EF5   ")) +
...
```{r}
grid.arrange(tornadoes_left, tornadoes_right, ncol = 2)
```
